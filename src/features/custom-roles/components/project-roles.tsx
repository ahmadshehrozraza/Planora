...
import { projectRoleSchema } from "../schemas";
...
} from "../api/use-project-roles";

import { useResetProjectRoleInviteCode } from "../api/use-reset-project-role";

import { PROJECT_LEVEL_PERMISSIONS } from "../hooks/roles-permissions";
...
export const ProjectRolesManager = () => {
    ...
    const { data: roles, isLoading } = useGetProjectRoles(projectId);
    const { mutate: createRole, isPending: isCreating } = useCreateProjectRole();
    const { mutate: updateRole, isPending: isUpdating } = useUpdateProjectRole();
    const { mutate: deleteRole, isPending: isDeleting } = useDeleteProjectRole();
    const { mutate: resetLink, isPending: isResetting } = useResetProjectRoleInviteCode();
    ...
    const [ConfirmResetDialog, confirmReset] = useConfirm(
        "Reset Invite Link",
        "Anyone with the old link will no longer be able to join this role. Continue?",
        "destructive"
    );

    const form = useForm<z.infer<typeof projectRoleSchema>>({
        resolver: zodResolver(projectRoleSchema),
        defaultValues: { name: "", workspaceId, projectId, permissions: [] },
    });

    const openModal = (role: any = null) => {
        setEditingRole(role);
        if (role) {
            form.reset({ name: role.name, workspaceId, projectId, permissions: role.permissions || [] });
        } else {
            form.reset({ name: "", workspaceId, projectId, permissions: [] });
        }
        setIsOpen(true);
    };
    ...
    const onSubmit = (values: z.infer<typeof projectRoleSchema>) => {
        if (editingRole) {
    ...
    const handleDelete = async (roleId: string) => {
        ...
        if (ok) {
            deleteRole({ roleId, projectId, workspaceId });
        }
    };
    ...
    const handleResetLink = async (roleId: string) => {
        ...
        if (ok) {
            resetLink({ roleId, projectId, workspaceId });
        }
    };
    ...
    const handleCopy = (roleToken: string) => {
        ...
        const url = `${window.location.origin}/workspaces/${workspaceId}/projects/${projectId}/join/${project.inviteCode}?t=${roleToken}`;
        navigator.clipboard.writeText(url);
        toast.success("Project role invite link copied!");
    };
            ...
            <CardHeader className="flex flex-row items-center justify-between border-b bg-muted/20">
                <div>
                    <CardTitle className="text-xl flex items-center gap-2">
                        <Shield className="size-5 text-primary" /> Project Roles
                    </CardTitle>
                    <CardDescription>Create and manage roles specific to this project.</CardDescription>
                </div>
                <Dialog open={isOpen} onOpenChange={handleOpenChange}>
                    <DialogTrigger asChild>
                        <Button onClick={() => openModal(null)} size="sm">
                            <Plus className="size-4 mr-2" /> New Project Role
                        </Button>
                    </DialogTrigger>
                            ...
                            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input {...field} placeholder="e.g. Lead Developer" disabled={isCreating || isUpdating} />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-1 gap-3 border p-4 rounded-lg bg-muted/10 max-h-[300px] overflow-y-auto">
                                        {PROJECT_LEVEL_PERMISSIONS.map((perm) => (
                                            <FormField
                                                key={perm.value}
                                                control={form.control}
                                                name="permissions"
                    ...
                    {roles?.map((role: any) => (
                        <div key={role.id} className="p-5 flex flex-col gap-4 hover:bg-muted/10 transition">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Button 
                                        variant="destructive" 
                                        size="sm" 
                                        disabled={role.isSystem || role.isProjectDefault || isDeleting} 
                            ...
                            <div className="flex items-center gap-2 bg-muted/40 p-2.5 rounded-lg border border-border/50 w-full max-w-xl">
                                ...
                                <Input 
                                    readOnly 
                                    value={project?.inviteCode ? `${window.location.origin}/workspaces/${workspaceId}/projects/${projectId}/join/${project.inviteCode}?t=${role.inviteCode}` : "Loading link..."} 
