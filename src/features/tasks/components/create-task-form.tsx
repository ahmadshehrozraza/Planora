...
export const CreateTaskForm = ({
    ...
    const { data: tasks, isLoading: isLoadingTasksApi } = useGetTasksByProject(selectedProjectId);
    ...
    // Watch projectId to filter segments and tasks
    ...
    useEffect(() => {
        if (watchProjectId && watchProjectId !== selectedProjectId) {
            ...
            loadSegmentsForProject(watchProjectId);
    ...
    const loadSegmentsForProject = (projectId: string) => {
        ...
        setTimeout(() => {
            ...
            const segments = projectSegments.map(segment => ({
                id: segment.id,
                name: segment.name,
            }));
            ...
            if (segments.length === 1 && !form.getValues("segmentId")) {
                form.setValue("segmentId", segments[0].id);
            }
        }, 300);
    };
