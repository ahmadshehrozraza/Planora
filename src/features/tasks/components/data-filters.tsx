...
export const DataFilters = ({
    ...
    const { data: segments, isLoading: isLoadingSegments } = useGetDummySegmentsByProject(
        isProjectContext ? projectId : (filterProjectId || undefined) 
    );

    const isLoading = isLoadingProjects || isLoadingMembers || (isProjectContext && isLoadingSegments);
    ...
    const currentProject = projects?.documents?.find(p => p.id === projectId);
    ...
    const onAssigneeChange = (value: string) => {
        setFilters({ assigneeId: value === "all-tasks" ? null : value });
    };
    ...
    const resetAllFilters = () => {
        setFilters({
            status: null,
            assigneeId: null,
            projectId: isProjectContext ? projectId : null, 
            segmentId: null,
            dueDate: null,
            search: null,
        });
