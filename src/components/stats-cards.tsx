  ...
  "flex items-center justify-between border shadow-sm transition-colors",
  {
    variants: {
      variant: {
        ...
        secondary: "bg-violet-50/50 border-violet-100 dark:bg-violet-500/10 dark:border-violet-500/20",
...
const valueVariants = cva("text-xl font-bold", {
  variants: {
    variant: {
      ...
      secondary: "text-violet-900 dark:text-violet-100",
      ...
      <CardContent className="p-4 w-full">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <div className={cn("p-2 rounded-lg bg-background/50 border border-background/20")}>
              <Icon className={iconVariants({ variant })} />
            </div>
            <p className={cn(textVariants({ variant }), "truncate")}>{label}</p>
          </div>
